...
import { Store, ClipboardList, DollarSign, Camera, TrendingUp, Clock, AlertCircle, Upload, Users } from 'lucide-react'
...
export default function EbayDashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: emp } = await supabase.from('users').select('*').or('platform.eq.ebay,platform.eq.both')
      setStores(s || [])
  ...
  // Team breakdown
  const ebayBoys = employees.filter(e => e.team_name === 'EBAY Boys').length
  const ebayGirls = employees.filter(e => e.team_name === 'EBAY Girls').length
  const ebayManagement = employees.filter(e => e.team_name === 'EBAY Management').length
    ...
    <div className="p-8">
      ...
      <div className="bg-white rounded-xl shadow-sm border p-6 mb-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-lg font-semibold">eBay Team</h2>
        </div>
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-blue-50 rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-blue-700">{ebayBoys}</div>
            <div className="text-sm text-blue-600">eBay Boys</div>
          </div>
          <div className="bg-pink-50 rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-pink-700">{ebayGirls}</div>
            <div className="text-sm text-pink-600">eBay Girls</div>
          </div>
          <div className="bg-purple-50 rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-purple-700">{ebayManagement}</div>
            <div className="text-sm text-purple-600">Management</div>
          </div>
        </div>
      </div>
