...
export default function EtsyFinancePage() {
  ...
  useEffect(() => {
    async function loadData() {
      const { data: storesData } = await supabase.from('stores').select('*').eq('platform', 'etsy')
      setStores(storesData || [])
      const { data: ordersData } = await supabase.from('orders').select('*, store:stores(*)').order('order_date', { ascending: false })
      setOrders(ordersData?.filter((o: any) => o.store?.platform === 'etsy') || [])
      setLoading(false)
    }
    loadData()
  }, [supabase])
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-gray-500">Revenue and performance metrics</p>
      </div>
