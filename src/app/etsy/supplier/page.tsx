...
import { Package, Printer, AlertCircle, CheckCircle, Store, Filter, Download } from 'lucide-react'
...
export default function SupplierPage() {
  ...
  const orderStoreNames = Array.from(new Set(orders.map(o => o.store_name).filter(Boolean))).sort()
  
  // Get unique countries from orders
  const orderCountries = Array.from(new Set(orders.map(o => o.country).filter(Boolean))).sort()
    ...
    <div className="p-8">
      ...
      <div className="flex justify-between items-start mb-6 print:hidden">
        <div>
          ...
          <p className="text-gray-500">
            {filteredOrders.length} orders ready to prepare
            {storeFilter && <span className="font-medium"> from {storeFilter}</span>}
            {countryFilter && <span className="font-medium"> for {countryFilter}</span>}
          </p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => downloadCSV('supplier_picklist', ['Order#','Store','SKU','Color','Size','Qty','Buyer','Address','City','Postcode','Country','Status'], filteredOrders.map(o => [o.order_number, o.store_name, o.sku, o.color, o.size, o.quantity, o.buyer_name, o.address, o.city, o.postcode, o.country, o.status]))}
      ...
      <div className="bg-white rounded-xl shadow-sm border p-4 mb-6 print:hidden">
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <option value="">All Stores ({orders.length} orders)</option>
              {orderStoreNames.map(name => {
                const count = orders.filter(o => o.store_name === name).length
                return (
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {orderCountries.map(country => {
                const count = orders.filter(o => o.country === country).length
                return (
          ...
          {(storeFilter || countryFilter) && (
            <button 
              onClick={() => { setStoreFilter(''); setCountryFilter(''); }}
        ...
        {/* Quick Store Buttons */}
        {orderStoreNames.length > 0 && orderStoreNames.length <= 10 && (
          <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t">
            ...
            {orderStoreNames.map(name => {
              ...
              return (
                <button
                  key={name}
                  onClick={() => setStoreFilter(storeFilter === name ? '' : name)}
      ...
      <div className="hidden print:block mb-6 text-center border-b pb-4">
        ...
        <p className="text-gray-600">
          Date: {new Date().toLocaleDateString()} | Total Orders: {filteredOrders.length}
        ...
        <div className="bg-white rounded-xl p-12 text-center">
          ...
          <p className="text-gray-500">
            {orders.length === 0 
              ...
              : `No orders match the current filters (${orders.length} total pending)`
            }
          </p>
        </div>
      ) : (
          ...
          {filteredOrders.map((order, index) => {
            ...
            return (
              <div key={order.id} className="bg-white rounded-xl shadow-sm border overflow-hidden print:break-inside-avoid print:shadow-none print:border-2">
                ...
                {/* Store Badge */}
                <div className="px-4 py-1 bg-gray-100 text-xs text-gray-600 flex justify-between">
