...
import { Palette, AlertCircle, Download } from 'lucide-react'
...
export default function EtsyGraphicsPage() {
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-start mb-8"><div><h1 className="text-2xl font-bold text-gray-900">Graphics Queue</h1><p className="text-gray-500">Track graphic design workflow</p></div><button onClick={() => downloadCSV('graphics_queue', ['SKU','Store','GD Stage','SEO Stage','Status'], queueItems.map(i => [i.sku, i.store, i.gdStage, i.seoStage, i.status]))} className="flex items-center gap-2 px-4 py-2 bg-white border rounded-lg text-sm hover:bg-gray-50"><Download className="w-4 h-4" />Export CSV</button></div>
