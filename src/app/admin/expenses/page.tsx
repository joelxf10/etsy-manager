...
import { Receipt, TrendingUp, Calendar, Filter, Download } from 'lucide-react'
...
export default function ExpensesPage() {
  ...
  const months = Array.from(new Set(expenses.map(e => e.month_year).filter(Boolean))).sort().reverse()
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-start mb-8">
        ...
        <button
          onClick={() => downloadCSV('expenses', ['Date','Category','Description','Amount PKR','Month'], expenses.map(e => [e.expense_date, e.category, e.description, e.amount, e.month_year]))}
