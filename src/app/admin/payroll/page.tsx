...
export default function PayrollPage() {
  ...
  const teams = Array.from(new Set(employees.map(e => e.team_name).filter(Boolean)))
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-start mb-8">
        ...
        <button onClick={() => downloadCSV('payroll', ['Name','Email','Role','Platform','Team','Department','Base Salary PKR','Active'], employees.map(e => [e.name, e.email, e.role, e.platform, e.team_name, e.department, e.base_salary, e.is_active ? 'Yes' : 'No']))} className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">
