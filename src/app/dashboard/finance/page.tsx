  ...
  const loadData = async () => {
    ...
    if (ordersData) {
      const pnl = ordersData.map(order => {
        ...
        const profitUsd = saleUsd - totalCostUsd
                ...
                {pnlData.length === 0 ? (
                  ...
                  pnlData.map(row => (
                    <tr key={row.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 font-mono text-xs">{row.store_sku}</td>
                      <td className="px-4 py-3">{row.qty}</td>
                      <td className="px-4 py-3">${row.sale_usd.toFixed(2)}</td>
                      <td className="px-4 py-3 text-red-600">${row.cost_usd.toFixed(2)}</td>
                      <td className="px-4 py-3 text-red-600">${row.shipping_usd.toFixed(2)}</td>
                      <td className="px-4 py-3 text-red-600">${row.total_cost_usd.toFixed(2)}</td>
