...
export default function ProductsPage() {
  ...
  const [newSku, setNewSku] = useState({ store_id: '', store_sku: '' })
  ...
  const loadProducts = async () => {
    ...
    if (productsData) {
      const productsWithData = productsData.map(p => ({
        ...
        skuMappings: skuData?.filter(s => s.product_id === p.id).map(s => ({
          ...s,
  ...
  const handleAddSkuMapping = async (productId: string) => {
    if (!newSku.store_id || !newSku.store_sku) {
      alert('Please select a store and enter SKU')
      return
    }

    const { error } = await supabase.from('store_skus').insert({
      store_id: newSku.store_id,
      product_id: productId,
      store_sku: newSku.store_sku,
    })
  ...
  const deleteSkuMapping = async (skuId: string) => {
    if (!confirm('Delete this SKU mapping?')) return
    await supabase.from('store_skus').delete().eq('id', skuId)
    loadProducts()
  }
      ...
      <div className="p-6">
        ...
        {showAddProduct && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl p-6 w-full max-w-md" data-tour="product-form">
        ...
        <div className="space-y-4">
          {products.length === 0 ? (
            ...
            products.map((product) => (
              <div key={product.id} className="bg-white rounded-xl border overflow-hidden" data-tour="product-expand">
                <div 
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-sm text-gray-400">
                      ({product.variants?.length || 0} variants, {product.skuMappings?.length || 0} SKUs)
                    </span>
                  </div>
                  ...
                  <div className="border-t bg-gray-50 p-4">
                    ...
                    <div className="mb-6">
                      ...
                      {showAddVariant === product.id && (
                        <div className="bg-white border rounded-lg p-4 mb-4" data-tour="variant-form">
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="text-left text-gray-500">
                              ...
                              {showCosts && <th className="pb-2">Cost USD</th>}
                              {showCosts && <th className="pb-2">Shipping USD</th>}
                    ...
                    <div data-tour="sku-mapping">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="font-medium text-sm text-gray-700">SKU Mappings</h4>
                        <button
                          onClick={() => setShowAddSku(product.id)}
                          className="text-sm text-emerald-600 hover:underline"
                        >
                      ...
                      {/* Add SKU Form */}
                      {showAddSku === product.id && (
                        <div className="bg-white border rounded-lg p-4 mb-4">
                          <div className="grid grid-cols-2 gap-3">
                            <select
                              value={newSku.store_id}
                              onChange={(e) => setNewSku({ ...newSku, store_id: e.target.value })}
                              className="border rounded px-2 py-1 text-sm"
                            >
                            ...
                            <input
                              value={newSku.store_sku}
                              onChange={(e) => setNewSku({ ...newSku, store_sku: e.target.value })}
                              placeholder="Store SKU (e.g., UMERNBS01-0001)"
                              className="border rounded px-2 py-1 text-sm"
                            />
                          </div>
                          <div className="flex justify-end gap-2 mt-3">
                            <button
                              onClick={() => setShowAddSku(null)}
                              className="px-3 py-1 text-sm border rounded hover:bg-gray-50"
                            >
                              Cancel
                            </button>
                            <button
                              onClick={() => handleAddSkuMapping(product.id)}
                              className="px-3 py-1 text-sm bg-emerald-500 text-white rounded hover:bg-emerald-600"
                            >
                              Add
                            </button>
                      ...
                      {product.skuMappings && product.skuMappings.length > 0 ? (
                        <div className="space-y-2">
                          {product.skuMappings.map((sku) => (
                            <div key={sku.id} className="flex items-center justify-between bg-white border rounded px-3 py-2">
                              <div>
                                <span className="text-sm font-mono">{sku.store_sku}</span>
                                <span className="text-xs text-gray-400 ml-2">({sku.store_name})</span>
                              </div>
                              <button
                                onClick={() => deleteSkuMapping(sku.id)}
                                className="text-gray-400 hover:text-red-500"
                              >
                                <i className="fas fa-times"></i>
                              </button>
                        ...
                        <p className="text-sm text-gray-400">No SKU mappings yet</p>
                      )}
                    </div>
                  </div>
                )}
