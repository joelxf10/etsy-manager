...
import { ShoppingBag, LayoutDashboard, Package, ClipboardList, DollarSign, Camera, Users, HelpCircle, LogOut, ChevronLeft, Store, FileText, BarChart3, Palette } from 'lucide-react'
    ...
    <div className="w-64 bg-slate-900 min-h-screen flex flex-col">
      ...
      {(user?.role === 'admin' || user?.role === 'manager') && (
        <div className="p-4 border-t border-slate-800">
          ...
          <Link href="/admin/costs" className="flex items-center gap-3 px-3 py-2 rounded-lg text-gray-400 hover:text-white hover:bg-slate-800">
            <BarChart3 className="w-5 h-5" />Costs Tracker
          </Link>
          <Link href="/admin/employees" className="flex items-center gap-3 px-3 py-2 rounded-lg text-gray-400 hover:text-white hover:bg-slate-800">
            <Users className="w-5 h-5" />Employees
          </Link>
          <Link href="/admin/reports" className="flex items-center gap-3 px-3 py-2 rounded-lg text-gray-400 hover:text-white hover:bg-slate-800">
