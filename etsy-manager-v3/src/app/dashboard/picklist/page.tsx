...
export default function PicklistPage() {
    ...
    if (ordersData) {
      const picklist = ordersData.map(order => {
        ...
        return {
          ...
          cost_usd: order.resolved_cost_usd,
          shipping_usd: order.resolved_shipping_usd,
        }
      })
      setItems(picklist)
    }
  ...
  const exportCSV = () => {
    const headers = ['Order Date', 'Order #', 'VAR ID', 'Qty', 'Color', 'Size', 'Supplier Variation', 'Cost USD', 'Shipping USD', '1688 Link']
  ...
  // Group by VAR_ID for summary
  const summaryMap: Record<string, { var_id: string; qty: number; supplier_variation: string | null; link: string | null }> = {}
  items.forEach(item => {
    if (!summaryMap[item.var_id]) {
      summaryMap[item.var_id] = {
        var_id: item.var_id,
        qty: 0,
        supplier_variation: item.supplier_variation,
        link: item.link_1688
      }
    }
    summaryMap[item.var_id].qty += item.qty
  })
    ...
    <div>
      ...
      <div className="p-6">
        ...
        <div className="bg-white rounded-xl border p-4 mb-6">
          <h3 className="font-semibold mb-3">Summary by Variant</h3>
          <div className="grid grid-cols-4 gap-3">
            {summary.slice(0, 8).map(s => (
              <div key={s.var_id} className="border rounded-lg p-3">
                <p className="font-mono text-sm text-emerald-600">{s.var_id}</p>
                <p className="text-2xl font-bold">{s.qty} units</p>
                <p className="text-xs text-gray-500">{s.supplier_variation || '-'}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Full Picklist */}
        <div className="bg-white rounded-xl border overflow-hidden" data-tour="picklist-table">
